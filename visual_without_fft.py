...
LOG_DIR = 'projector_without_fft'
NAME_TO_VISUALISE_VARIABLE = "raw_embedding"
FEATURE_TRAIN_PATH='feature_test_e.csv'
TO_EMBED_COUNT = 20     
BATCH_SIZE=80
LENS=640
...
Data_feature=pd.read_csv(FEATURE_TRAIN_PATH)
...
def build_network():
    ...
    with tf.name_scope('train'):
        optimize = tf.train.AdamOptimizer(learning_rate=learning_rate_new).minimize(cost)
...
def visualize():
    ...
    with open('embed.tsv','w') as f:
        ...
        for index,label in enumerate(labels):
            f.write("%d\t%d\n" % (index,label))
    y=tf.Variable(feature2,name=NAME_TO_VISUALISE_VARIABLE)
    writer = tf.summary.FileWriter("projector_without_fft/")
    config      =  projector.ProjectorConfig()
    embedding = config.embeddings.add()
    ...
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
    saver.save(sess, os.path.join(LOG_DIR, "model1.ckpt"), 1)
    writer.close
def visualize2():
    data=Data
    data=np.array(data)
    ...
    feature2=np.array(feature2)
    labels=data[:,-1]
    with open('embed_without_fft.tsv','w') as f:
        f.write("Index\tLabel\n")
        for index,label in enumerate(labels):
            f.write("%d\t%d\n" % (index,label))
    y=tf.Variable(feature2,name="raw_data_without_fft")
    writer = tf.summary.FileWriter("projector_without_fft/")
    config      =  projector.ProjectorConfig()
    embedding = config.embeddings.add()
    embedding.tensor_name = y.name
    embedding.metadata_path = 'embed_without_fft.tsv' #'metadata.tsv'
    projector.visualize_embeddings(writer, config)
    sess=tf.InteractiveSession()
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
...
def visualize_all():
    ...
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
    saver.save(sess, os.path.join(LOG_DIR, "embed_without_fft.ckpt"), 1)
    ...
    feature2=np.array(feature2)
    labels=data[:,0]
    with open('embed_feature','w') as f:
        f.write("Index\tLabel\n")
        for index,label in enumerate(labels):
            f.write("%d\t%d\n" % (index,label))
    y=tf.Variable(feature2,name="raw_data_feature")
    ...
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
    saver.save(sess, os.path.join(LOG_DIR, "raw_data_feature.ckpt"), 1)
    writer.close
...
def train_network(graph,pb_file_path,batch_size=BATCH_SIZE,num_epochs=EPOCHS):
    ...
    writer = tf.summary.FileWriter("projector_without_fft/", sesss.graph)
    # t=train_data2(True)
    # for (i,j) in t:
    #     x=i
    #     y=j
