...
def visualize3():   
    ...
    writer = tf.summary.FileWriter("projector_feature/")
...
def build_network():
    ...
    with tf.name_scope('layer2'):
        W_conv6 = weight_variable([1,2, 16, 64])
        b_conv6 = bias_variable([64])
        tf.summary.histogram('weight',W_conv6)
        tf.summary.histogram('bias',b_conv6)
        h_conv6 = tf.nn.relu(conv2d_1(h_pool1, W_conv6) + b_conv6)
...
def train_network(graph,pb_file_path,batch_size=BATCH_SIZE,num_epochs=EPOCHS):
    ...
    writer = tf.summary.FileWriter("projector_feature/", sesss.graph)
    with tf.Session(config=config) as sess:
        ...
        #saver.save(sess,"./logs_feature/model.ckpt")   
